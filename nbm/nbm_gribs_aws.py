...
def s3_to_temp(obj, compress=False):
    ...
    try:
        ...
        elif ((compress == True) & os.path.isfile(subset_gz)):
            print('Compressed Grib exists, skipping %s'%obj_init)

        elif ((compress == False) & (os.path.isfile(subset))):
            print('Grib exists, skipping %s'%obj_init)
            pass
...
start_4p0 = datetime(2020, 10, 15, 0, 0)
